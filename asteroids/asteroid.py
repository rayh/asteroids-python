...
class Asteroid(Particle):
    ...
    def randomized(screen_size: Tuple[float, float]) -> Asteroid:
        velocity_m = Asteroid.MIN_SPEED + (
            random() * (Asteroid.MAX_SPEED - Asteroid.MIN_SPEED)
        )
        velocity_v = Vec2d(
            cos(random() * 2 * pi) * velocity_m, sin(random() * 2 * pi) * velocity_m
        )
        ...
        return Asteroid(mass=mass, velocity=velocity_v, position=position)
    ...
    def __init__(self, mass, velocity, position):
        ...
        super(Asteroid, self).__init__(mass=mass, vertices=self.poly.points)
    ...
    def on_collision(self, scene: Scene, by: Particle, ke: float) -> bool:
        ...
        self.spawn_smaller_asteroids(scene, ke=ke)

        return True

    def spawn_smaller_asteroids(self, scene: Scene, ke: float = 0):
        fragments = randint(2, 6)
